    ...
    Post.findById(req.body.postId).exec((err, post) => {
        ...
        if (post) {
            if (post.comments.length > 0) {
                var _comment = post.comments.find((comment) => comment._id == req.params['commentId']);
                if (_comment != null) {
...
exports.updateCommentDetails = (req, res) => {
    Post.findById(req.body.postId).exec(async (err, post) => {
        ...
        if (post) {
            if (post.comments.length > 0) {
                var _commentIndex = post.comments.findIndex((comment) => comment._id == req.params['commentId']);
                if (_commentIndex >= 0) {
    ...
    Post.findById(req.params['postId']).exec(async (err, post) => {
        ...
        if (post) {
            if (post.comments.length > 0) {
                var _commentIndex = post.comments.findIndex((comment) => comment._id == req.params['commentId']);
                if (_commentIndex >= 0) {
                    await Post.updateMany({ _id: req.params['postId'] }, { $pull: { comments: { _id: req.params['commentId'] } } });
